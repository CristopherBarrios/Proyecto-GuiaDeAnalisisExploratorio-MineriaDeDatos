...
library (ggplot2)
divorciosOcupHombre <- subset(divorcios, CIUOHOM)
...
## Escolaridad en Mujeres 2009-2021
```{r normalHistEdadHombre, echo=FALSE}
#install.packages("ggplot2",repos = "http://cran.us.r-project.org")
...
library (ggplot2)
divorciosOcupMujeres <- subset(divorcios, CIUOMUJ)
...
```{r kmeans, echo=FALSE}
datosClustering <- divorcios[, c("EDADHOM", "EDADMUJ", "MESREG")]
datosNormalizados <- scale(datosClustering)
set.seed(123)
modeloKMeans <- kmeans(datosNormalizados, centers = 4)
library(plotly)
plot_ly(datosClustering, x = ~EDADHOM, y = ~EDADMUJ, z = ~MESREG, color = modeloKMeans$cluster) %>% 
  add_markers() %>% 
