...
data <- select(D2009, Depreg, Mupreg)

# Convertir variables a formato adecuado para clustering
data_bin <- model.matrix(~ Depreg + Mupreg - 1, data = data)

# Realizar clustering con k-modes
set.seed(123)
kmodes_model <- clustMixType::kmod(data_bin, 3)

# Mostrar resultados del clustering
fviz_cluster(kmodes_model, data = data_bin)
